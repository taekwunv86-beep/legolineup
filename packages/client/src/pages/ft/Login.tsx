...
export default function FtLogin() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (res.user.role !== "ft") {
        setError("FT 계정이 아닙니다.");
        return;
      }
      saveSession(res.token, res.user);
      navigate("/ft", { replace: true });
    } catch (e) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-100 p-4">
      ...
      >
        ...
        <h1 className="mb-1 text-2xl font-bold">FT 로그인</h1>
        <p className="mb-6 text-sm text-slate-500">현장 운영자 계정으로 로그인하세요.</p>
        ...
        <label className="mb-3 block">
          ...
          <input
            ...
            required
            className="w-full rounded-lg border border-slate-300 px-3 py-3 text-lg outline-none focus:border-slate-900"
          />
        </label>
